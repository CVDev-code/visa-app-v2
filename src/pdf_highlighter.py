...
def _rect_has_ink_adaptive(page: fitz.Page, rect: fitz.Rect, *, dpi=INKCHECK_DPI, sample_stride=INK_SAMPLE_STRIDE, delta=INK_DELTA, ratio_max=INK_RATIO_MAX) -> bool:
    r = (fitz.Rect(rect) & page.rect)
    if r.is_empty or r.width < 2 or r.height < 2: return True
    pix = page.get_pixmap(matrix=fitz.Matrix(dpi/72, dpi/72), clip=r, alpha=False)
    s, w, h = pix.samples, pix.width, pix.height
    if not s or w < 2 or h < 2: return True
    ...
    for y in range(0, h, step):
        ...
        for x in range(0, w, step):
            i = row + x * 3
            sr += s[i]; sg += s[i+1]; sb += s[i+2]; total += 1
    ...
    for y in range(0, h, step):
        ...
        for x in range(0, w, step):
            i = row + x * 3
            if (abs(s[i]-br)>delta) or (abs(s[i+1]-bg)>delta) or (abs(s[i+2]-bb)>delta): ink += 1
            total2 += 1
...
def _page_text_shapes(page: fitz.Page) -> List[fitz.Rect]:
    ...
    try:
        ...
        for w in page.get_text("words"): rects.append(fitz.Rect(w[:4]))
        d = page.get_text("dict")
        for b in d.get("blocks", []):
            for ln in b.get("lines", []):
                bbox = ln.get("bbox")
                if bbox: rects.append(inflate_rect_xy(fitz.Rect(bbox), LINE_BLOCKER_PAD_X, LINE_BLOCKER_PAD_Y))
    except: pass
    return rects
...
def _zones(page: fitz.Page) -> List[Tuple[str, fitz.Rect]]:
    ...
    for gw in GUTTER_WIDTHS:
        ...
        if left.width > 40: zones.append((f"left_{int(gw)}", left))
        if right.width > 40: zones.append((f"right_{int(gw)}", right))
    if env:
...
def _choose_best_spot(page, targets, occupied_callouts, label):
    ...
    tc, blockers = _center(target_union), _page_blockers(page)
    ...
    conn_obs = [inflate_rect(b, 1.5) for b in _page_text_shapes(page)] + \
    ...
    def find_in_zones(zone_list):
        ...
        for zn, zr in zone_list:
            ...
            bw = min(210.0, max(90.0, zr.width - 8.0)) if is_gutter else min(300.0, max(140.0, zr.width - 12.0))
            fs, wrapped, w, h = _optimize_layout(label, bw)
            
            y_steps = _grid_positions(zr, w, h, step=18.0)
            y_steps.sort(key=lambda p: abs(p.y - tc.y) + 0.2 * abs(p.x - tc.x))
            
            for c in y_steps[:950 if is_gutter else 550]:
                cand = fitz.Rect(c.x-w/2, c.y-h/2, c.x+w/2, c.y+h/2)
                if _intersects_any(cand, blockers + occupied_buf + highlight_blockers): continue
                
                ratio = INK_RATIO_MAX_GUTTER if is_gutter else INK_RATIO_MAX
                if _rect_has_ink_adaptive(page, inflate_rect(cand, INKCHECK_PAD), ratio_max=ratio): continue
                
                s, e, hits, length = _straight_connector_best_pair(cand, target_union, conn_obs)
                # Lower penalty (2500) so that crossing a column doesn't break the side-bias
                score = (hits * 2500.0) + length + abs((c.y - tc.y) * 0.7)
                if is_gutter: score -= 10000.0 # Substantial side-bias
                cands.append((score, cand, wrapped, fs, True))
        return sorted(cands, key=lambda x: x[0])

    res = find_in_zones(side_zones)
    if not res: res = find_in_zones(other_zones)
    
    if res: return res[0][1], res[0][2], res[0][3], res[0][4]
    
    # Ultimate Fallback
    fs, wrapped, w, h = _optimize_layout(label, 170.0)
    n = len(occupied_callouts)
    fb = fitz.Rect(EDGE_PAD + (n%6)*20, EDGE_PAD + (n//6)*40, EDGE_PAD+w, EDGE_PAD+h)
...
def _search_term(page: fitz.Page, term: str) -> List[fitz.Rect]:
    t = (term or "").strip()
    if not t: return []
    if len(t) > _MAX_TERM: t = t[:_MAX_TERM]
    ...
    if not rs and len(t) >= _CHUNK:
        ...
        for i in range(0, len(t), step):
            ...
            if len(chunk) >= 18: hits.extend(page.search_for(chunk))
        if hits:
            hits.sort(key=lambda r: (r.y0, r.x0))
            merged = []
            for r in hits:
                if not merged: merged.append(fitz.Rect(r))
                else:
                    if merged[-1].intersects(r) or abs(merged[-1].y0 - r.y0) < 3.0:
                        merged[-1] |= r
                    else: merged.append(fitz.Rect(r))
            return merged
    return rs
...
def annotate_pdf_bytes(pdf_bytes: bytes, quote_terms: List[str], criterion_id: str, meta: Dict) -> Tuple[bytes, Dict]:
    ...
    def _do_job(label, needles, policy="union"):
        ...
        obs = [inflate_rect(r, 1.5) for r in _page_text_shapes(page1)] + \
              [inflate_rect(o, 2.0) for o in occ] + [inflate_rect(t, 2.5) for t in tgs]
    ...
    if meta.get("performance_date"): _do_job("Performance date.", [meta.get("performance_date")])
    if meta.get("salary_amount"): _do_job("Beneficiary salary evidence.", [meta.get("salary_amount")])
